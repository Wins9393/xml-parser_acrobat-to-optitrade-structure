...
let tabClientsNumber = [
  ...
  { "UNIQUE OPTIEK & OPTOMETRIE": 181100 },
];
...
const formatJson = async (json) => {
  ...
  // Numéro de document
  const numeroDoc =
    formatedJson[0]["rsm:CrossIndustryInvoice"]["rsm:ExchangedDocument"][0][
      "ram:ID"
    ][0];
...
const processCentropData = async (req, res) => {
  ...
  try {
    ...
    for await (const part of parts) {
      ...
      if (part.fieldname === "inputFileNumber") {
        fields.inputFileNumber = part.value;
      }

      if (part.fieldname === "testProduction") {
        fields.testProduction = part.value;
      }
